...
REGISTER_PLUGIN_BASIC(OpticksEdgeDetection, Prewitt);
...
namespace
{
   ...
   void edgeDetection(T* pData, DataAccessor pSrcAcc, int row, int col, int rowSize, int colSize)
   {
      ...
      double gx = -1.0 * upperLeftVal + -1.0 * leftVal + -1.0 * lowerLeftVal + 1.0 * upperRightVal + 1.0 *
         rightVal + 1.0 * lowerRightVal;
      double gy = -1.0 * lowerLeftVal + -1.0 * downVal + -1.0 * lowerRightVal + 1.0 * upperLeftVal + 1.0 *
...
bool Prewitt::getInputSpecification(PlugInArgList*& pInArgList)
{
...
bool Prewitt::getOutputSpecification(PlugInArgList*& pOutArgList)
{
...
bool Prewitt::execute(PlugInArgList* pInArgList, PlugInArgList* pOutArgList)
{
   StepResource pStep("Prewitt Edge Detection", "app", "81C41FDB-A520-467A-9CC9-DC364F415D0D");
   ...
   pOutArgList->setPlugInArgValue("Prewitt_Edge_Result", pResultCube.release());

...
REGISTER_PLUGIN_BASIC(OpticksEdgeDetection, Laplace);
...
namespace
{
   ...
   void edgeDetection(T* pData, DataAccessor pSrcAcc, int row, int col, int rowSize, int colSize)
   {
      ...
      double magnitude = -1.0 * upperLeftVal + -1.0 * leftVal + -1.0 * lowerLeftVal + -1.0 * upperRightVal + -1.0 *
         rightVal + -1.0 * lowerRightVal + -1.0 * downVal + -1.0 * upVal + 8.0 * midVal;
...
Laplace::Laplace()
{
   setDescriptorId("{F838856B-AEE8-40AD-81C0-DA7749680F26}");
   setName("Laplace Edge Detection");
   setVersion("Sample");
   ...
   setMenuLocation("[Edge Detection]/Laplacian Filter");
...
bool Laplace::getInputSpecification(PlugInArgList*& pInArgList)
{
...
bool Laplace::getOutputSpecification(PlugInArgList*& pOutArgList)
{
...
bool Laplace::execute(PlugInArgList* pInArgList, PlugInArgList* pOutArgList)
{
   StepResource pStep("Laplacian Edge Detection", "app", "AE2669DE-4FB8-43B3-A9CE-57F860E5FDE7");
   ...
   pOutArgList->setPlugInArgValue("Laplace_Edge_Result", pResultCube.release());
